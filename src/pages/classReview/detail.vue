    ...
    <view class="content">
      ...
      <view class="mt-68rpx">
          ...
          <view v-for="(item, index) in resourceList" :key="index" class="classroom-resources">
            <view>
              <img
                ...
                :src="getModuleTypeIcon(item.contentType, item.fileFormat)"
                alt=""
              />
              <text class="w-336rpx truncate">{{ item.title }}</text>
            </view>
            <text class="btn" @click="previewResources(item)">查看</text>
          </view>
        </view>
        <view v-else>
          <hEmpty text="暂无课堂资源" />
        ...
        <div class="course-video" v-if="reachResourcesDetailList.filter((i) => i.type == 1).length">
          <view
            v-for="(item, index) in reachResourcesDetailList.filter((i) => i.type == 1)"
            :key="index"
        ...
        <div class="course-video" v-if="reachResourcesDetailList.filter((i) => i.type == 2).length">
          <view
            v-for="(item, index) in reachResourcesDetailList.filter((i) => i.type == 2)"
            :key="index"
            class="course-video-list"
          >
...
<script setup>
  ...
  import {
    getClassCoursesBaseData,
    getClassCoursesHistoryResourceList,
    getTeachResources,
  } from './service'
