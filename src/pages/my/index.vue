    ...
    <view class="header">
      ...
      <view class="flex flex-col flex-1" style="color: #fff">
        <view class="flex items-center justify-between">
          ...
          <img class="w-40rpx h-40rpx" @click="settingDetail" src="@/static/my/setting_icon.png" alt="" />
        </view>
        <text class="mt-30rpx text-30rpx">广州市西关外国语学校-学生</text>
      </view>
    </view>
    <view class="user-info">
      <view class="card-list" v-for="(item, index) in cardList" :key="index" @click="showToast(item, index)">
        <view>
          <img class="w-60rpx h-60rpx mr-20rpx" :src="item.icon" alt="" />
          <text>{{ item.name }}</text>
        </view>
        <img class="w-28rpx h-28rpx cursor-pointer" v-if="!item.phone" src="@/static/my/arrow-right.png" alt="" />
