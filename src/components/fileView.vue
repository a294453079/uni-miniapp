<template>
  ...
  <view class="file-view" v-if="resourceInfo != null">
    ...
    <view class="view-box" v-else>
      <!-- 网页预览 -->
      <web-view v-if="viewType(resourceInfo.fileFormat) == 1" class="web-view" :src="resourceUrl"></web-view>
      <!-- 图片预览 -->
      <image class="w-full" v-if="viewType(resourceInfo.fileFormat) == 4" :src="resourceUrl" @tap="preview"
        mode="scaleToFill"></image>
      <!-- 视频预览 -->
      <!-- 无法预览格式rmvb -->
      <view v-if="viewType(resourceInfo.fileFormat) == 2 && isVideoView !== null">
        <video v-if="isVideoView" class="w-full" ref="videos" id="myVideo" :src="resourceUrl"
          initial-time="initial_time" autoplay></video>
        <view v-if="!isVideoView">该视频资源格式无法预览</view>
      </view>
      <!-- 音频预览 -->
      <audio id="myAudio" class="w-full" @tap="audioClick" :src="resourceUrl"
        v-if="viewType(resourceInfo.fileFormat) == 3" controls autoplay></audio>
    </view>
  </view>
</template>
<script setup>
